...
def main(args):
    vector_space = {}
    file_list_vs = open("AnalisisGenerados/vector_space.txt", "r")
    vector_space = JSONDecoder().decode(file_list_vs.read())
    file_list_vs.close()

    documents = {}
    file_list_documents = open("AnalisisGenerados/documentoMatWS.txt", "r")
    documents = JSONDecoder().decode(file_list_documents.read())
    file_list_documents.close()

    dic = []
    file_dic = open("AnalisisGenerados/wordsGen.txt", "r")
    ...
    file = open("AnalisisGenerados/tfidf.txt","w")
    file.write(JSONEncoder().encode(tfidf))
    file.close()

    file = open("AnalisisGenerados/inv_frec_vector.txt","w")
